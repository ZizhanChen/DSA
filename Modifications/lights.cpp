...
#define HAL_LIBRARY_PATH2 "/dev/semantic_brightness2"
...
/*****************************************************************************/
#define BACKLIGHT_PATH1  "/sys/class/backlight/rk28_bl/brightness"
#define BACKLIGHT_PATH "/sys/class/backlight/backlight/brightness" // for kernel 4.4
...
static int rgb_to_brightness(struct light_state_t const *state)
{
    ...
    //12.20 test
    ALOGE("brightness rgb_to_light light: %d", (int)brightness);
    return brightness;
}
...
int set_backlight_light(struct light_device_t* dev, struct light_state_t const* state)
{
    ...
    //12.20 test
    ALOGE("brightness back light: %d", brightness);
    ALOGE("brightness semantic dump:");
    //semantic_dump(semantic_state);
    /*
    unsigned int *dataPtr = NULL;
    dataPtr = (unsigned int *)malloc(sizeof(int));
    for(unsigned int i=0;i<1;i++)
    {
        dataPtr[i] = i;
        ALOGE("brightness input: %d", dataPtr[i]);
    }
    FILE *fp = fopen(HAL_LIBRARY_PATH1,"w");
    fwrite(dataPtr,sizeof(int),1,fp);
    fclose(fp);
    free(dataPtr);

    FILE *fpr = NULL;
	fpr = fopen(HAL_LIBRARY_PATH1,"r");
	int brt;
	int readCnt = fread(&brt,sizeof(brt),1,fpr); 
	//int readCnt = fread(buffer,1,sizeof(buffer),fp);  
	printf("brightness readCnt = %d read: %d\n",readCnt, brt);
	fclose(fpr);
    */

    int fd = open(HAL_LIBRARY_PATH1, O_RDWR|O_CREAT, S_IRUSR | S_IRGRP | S_IROTH);
    ...
    if (fd >= 0) {
        ...
        int bytes = sprintf(buffer, "%d\n", value);
        int amt = write(fd, buffer, bytes);
        ALOGE("brightness write: %d %s", value, buffer);
        close(fd);
    }
    else
    {
        ALOGE("brightness write open failed: %d", fd);
    }
