        ...
        for (size_t i = 0; i < NUM_DISPLAY_ATTRIBUTES - 1; i++) {
            switch (DISPLAY_ATTRIBUTES[i]) {
                ...
                case HWC_DISPLAY_HEIGHT:
                    ...
                    //12.13 test
                    ALOGE("HWC_hwc1  original height %d", config.height);
                    config.height = 2160;
                    ALOGE("HWC_hwc1  modified height %d", config.height);
                    break;
...
status_t HWComposer::createWorkList(int32_t id, size_t numLayers) {
    ...
    if (mHwc) {
        ...
        if (hwcHasApiVersion(mHwc, HWC_DEVICE_API_VERSION_1_1)) {
            ...
            ALOGE("displayFrame.left %d, displayFrame.top, displayFrame.right %d, displayFrame.bottom", disp.framebufferTarget->displayFrame.left, disp.framebufferTarget->displayFrame.right);
...
public:
    ...
    virtual void setFrame(const Rect& frame) {
        ...
        ALOGE("set before displayFrame.left %d, displayFrame.top, displayFrame.right %d, displayFrame.bottom", getLayer()->displayFrame.left, getLayer()->displayFrame.right);


        //test 1.10
        /*if(getLayer()->displayFrame.left >= 240){
            getLayer()->displayFrame.left -= 240;
            getLayer()->displayFrame.right -= 240;
        }
        
        ALOGE("set after displayFrame.left %d, displayFrame.top, displayFrame.right %d, displayFrame.bottom", getLayer()->displayFrame.left, getLayer()->displayFrame.right);
        */
    }
#if RK_VR
